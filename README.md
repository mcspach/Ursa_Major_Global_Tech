...
**`unlisted: true`** (services) means the entry keeps its page, so existing
links and anything already sent out still resolve, but it disappears from the
services index and the homepage grid. Use this to retire an offer without
creating a dead URL. Filtering happens at the two grid queries;
`[slug].astro` deliberately does *not* filter, which is what keeps the page
reachable.

**`placeholder: true`** (products) marks invented filler content that must be
reviewed or removed before launch.

**`priceGate: true`** (offers, landing with the offering rework) is a feature
flag for pricing. The real figure stays in the content file, but every surface
that renders a price checks this flag first and prints "Pricing on request"
instead.
